...
class TablaPostgre:
    ...
    def __init__(self, usuario=getpass.getuser(),
        ...
        self.lista_columnas = ["item TEXT UNIQUE NOT NULL",
            ...
            for x in lista_columnas:
                if (x[:5].lower() != "item " and
                   ...
                   x[:10].lower() != "fecha_act ") or \
                    ...
                    self.lista_columnas.append(x.lower())

        self.__sql_interno("CREATE TABLE IF NOT EXISTS " + self.nombre_tabla + " (" +
                           ','.join(self.lista_columnas) + ")")

        print("Tabla '" + self.nombre_tabla.upper() +
              "' generada con las siguientes columnas: " + str(self.lista_columnas).upper())
    ...
    def eliminar(self):
        ...
        +-------------------------------------------------------------------------------+
        """
        if self.__sql_interno("SELECT EXISTS(SELECT 1 FROM pg_catalog.pg_class AS c " +
            ...
            if not self.__sql_interno("SELECT EXISTS(SELECT 1 FROM pg_catalog.pg_class AS c " +
                                      "JOIN pg_catalog.pg_namespace AS n " +
                                      "ON n.oid = c.relnamespace " +
                                      "WHERE c.relname = '" + self.nombre_tabla + "' AND " +
                                      "c.relkind = 'r');")[0][0]:
    ...
    def agregar_item(self, **kwargs):
        ...
        +-------------------------------------------------------------------------------+
        """
        t = [x.lower() for x in self.columnas()]

        item = ("'" + str(kwargs.get("item")) + "'" if
                isinstance(kwargs.get("item"), str) else
                "NULL" if kwargs.get("item") is None else
                str(kwargs.get("item")))
        ...
        if kwargs.get("item") is not None and \
            int(self.__sql_interno("SELECT COUNT(*) FROM " +
                self.nombre_tabla +
                " WHERE item = " + item)[0][0]) > 0:
    ...
    def eliminar_item(self, item):
        ...
        +-------------------------------------------------------------------------------+
        """
        if int(self.__sql_interno("SELECT COUNT(*) FROM " +
                                  self.nombre_tabla +
                                  " WHERE item = '" + item + "'")[0][0]) > 0:
    ...
    def actualizar_item(self, llave, **kwargs):
        ...
        elif int(self.__sql_interno("SELECT COUNT(*) FROM " +
                                    self.nombre_tabla +
                                    " WHERE item = '" +
                                    llave + "'")[0][0]) <= 0:
    ...
    def agregar_columna(self, columna):
        ...
        if not self.__sql_interno("SELECT column_name FROM information_schema.columns " +
            ...
            if not not self.__sql_interno("SELECT column_name FROM information_schema.columns " +
                                          "WHERE table_name = '" + self.nombre_tabla + "' AND " +
                                          "column_name='" + c[0] + "'"):
